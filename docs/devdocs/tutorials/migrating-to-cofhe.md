...
Sealoutput will be available through CoFHE.js only, make sure to allow the issuer of the permit, in order to be able to request sealoutput later. \
...
With CoFHE.js the encryption is done asynchronously, for this reason we can provide a callback function to log the encryption state (read about it [here](../cofhejs/encryption-operations.md))
